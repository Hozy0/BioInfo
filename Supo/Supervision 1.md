...
5. ![[Pasted image 20231111050127.png]]
7. The input of the small parsimony problem is a rooted binary tree with each leaf labelled by a single symbol, the output is a labelling of all other nodes of the tree by single symbols that minimises the tree’s parsimony score. It’s complexity is $O(mnk^2)$ where $m$ is the number of species, $n$ is the number of characters and $k$ is the number of states
8. The greedy heuristic for large parsimony aims to construct a phylogenetic tree by iteratively adding symbols in a way that minimises the total number of mutations. At each step, the algorithm considers all possible ways to add a new symbol and selects the option that leads to the least increase in the overall number of mutations.

## Multiple alignment
1. Dynamic programming, greedy, and progressive approaches are distinct strategies for aligning multiple sequences. Dynamic programming, characterised by its optimal substructure, computes an alignment matrix to find the optimal global or local alignment with a time complexity of $O(k\times n^2)$. Greedy algorithms, while faster at $O(k\times n\times m)$, might not yield the global optimum as they construct alignments iteratively. With a time complexity of $O(k\times n \times log⁡k)$, Progressive alignment builds a guide tree and aligns sequences progressively from leaves to the root, leveraging pairwise alignments for intermediate steps. They all have a space complexity of the same space complexity.
2. CLUSTALW is a multiple sequence alignment algorithm. Its input is a set of sequences in fasta format. The algorithm performs pairwise alignments to build the guide tree and then progresses to align the sequences following the guide tree structure. A final refinement is done using profile-profile alignment. The output is the multiple sequence alignment.
3. Assigning a quality score to a multiple sequence alignment involves assessing its reliability and accuracy. Conservation scores evaluate the degree of residue conservation, indicating potential functional regions. Gap scores assess the impact of gaps in the alignment, which is crucial for understanding variability. Statistical measures and benchmarking against known alignments offer quantitative evaluations. Considering phylogenetic consistency ensures alignment and coherence with expected evolutionary relationships. Visual inspection remains essential, allowing researchers to identify well-aligned regions and potential errors. The quality score often combines various measures, comprehensively evaluating the alignment's accuracy and biological relevance.